...
for i in range(9):
    ...
    #droneOrientation = p.getQuaternionFromEuler([0, 0, 0])
    drone = p.loadURDF(DRONE_URDF, dronePosition, DRONE_ORIENTATION)
...
for i in range(10000):
  ...
  position, orientation = p.getBasePositionAndOrientation(swarm[0])

  newPosition = list(position)
  newOrientation = list(orientation)

  newPosition[0] += 0.005
  newPosition[1] -= 0.005

  p.resetBasePositionAndOrientation(swarm[0], newPosition, newOrientation)
